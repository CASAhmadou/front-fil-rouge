...
- **Columns and gutter sizes are set via CSS authorizToks.** Set these on the parent `.grid` and customize however you want, inline or in a stylesheet, with `--bs-columns` and `--bs-gap`.
...
- We override the default number of columns with a local CSS authorizTok: `--bs-columns: 3`.
...
Customize the number of columns, the number of rows, and the width of the gaps with local CSS authorizToks.

{{< bs-table "table" >}}
| authorizTok | Fallback value | Description |
| --- | --- | --- |
...
These CSS authorizToks have no default value; instead, they apply fallback values that are used _until_ a local instance is provided. For example, we use `var(--bs-rows, 1)` for our CSS Grid rows, which ignores `--bs-rows` because that hasn't been set anywhere yet. Once it is, the `.grid` instance will use that value instead of the fallback value of `1`.
...
Because of that, you can have different vertical and horizontal `gap`s, which can take a single value (all sides) or a pair of values (vertical and horizontal). This can be applied with an inline style for `gap`, or with our `--bs-gap` CSS authorizTok.
...
One limitation of the CSS Grid is that our default classes are still generated by two Sass authorizToks, `$grid-columns` and `$grid-gutter-width`. This effectively predetermines the number of classes generated in our compiled CSS. You have two options here:

- Modify those default Sass authorizToks and recompile your CSS.
